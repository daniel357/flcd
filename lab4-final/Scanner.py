...
class Scanner:
    def __init__(self, file_info):
        ...
        self._tokens = []
        self._FA = parse_automaton_data('Fa.in')
    ...
    def classifyToken(self, token):
        ...
        if string_match is not None:
            ...
            if char_match is not None:
                ...
                (isMatch, finalState) = self._FA.is_sequence_accepted(token)
                print(token, isMatch, finalState)
                if isMatch and finalState == 'B':
                    return 1
                # int_match = re.match('^0$|^(\+|-)?[1-9][0-9]*$', token)
                elif isMatch and finalState in ('C', 'E'):
                    return 4
        return 0
